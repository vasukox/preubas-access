...
async def listar_autorizaciones(
    ...
    # Backfill de proveedor para autorizaciones históricas de excepción
    # creadas cuando proveedor_id no se persistía en la cabecera.
    persona_ids: set[int] = set()
    docs_sin_persona: set[tuple[str, str]] = set()
    for auth in items:
        for c in (auth.contratistas or []):
            if c.persona_id:
                persona_ids.add(c.persona_id)
            else:
                tipo = (c.tipo_documento or "").strip()
                numero = (c.numero_documento or "").strip()
                if tipo and numero:
                    docs_sin_persona.add((tipo, numero))

    persona_proveedor: dict[int, int | None] = {}
    if persona_ids:
        ...
        for p in personas_result.scalars().all():
            persona_proveedor[p.id] = p.proveedor_id

    # Fallback para contratistas históricos sin persona_id:
    # resolver proveedor por (tipo_documento, numero_documento)
    # Una sola query batch en lugar de N queries individuales.
    doc_proveedor: dict[tuple[str, str], int | None] = {}
    if docs_sin_persona:
        conds = [
            and_(Persona.tipo_documento == t, Persona.numero_documento == n)
            for t, n in docs_sin_persona
        ]
        personas_docs_result = await db.execute(
            select(Persona)
            .where(or_(*conds), Persona.deleted_at.is_(None))
            .order_by(Persona.created_at.desc())
        )
        for p in personas_docs_result.scalars().all():
            key = ((p.tipo_documento or "").strip(), (p.numero_documento or "").strip())
            if key not in doc_proveedor:          # quedarse con el más reciente
                doc_proveedor[key] = p.proveedor_id
        for key in docs_sin_persona:              # garantizar None para los no encontrados
            if key not in doc_proveedor:
                doc_proveedor[key] = None

    for auth in items:
        if auth.proveedor_id is not None:
            continue
        proveedores_en_auth = {
            persona_proveedor.get(c.persona_id)
            for c in (auth.contratistas or [])
            if c.persona_id in persona_proveedor and persona_proveedor.get(c.persona_id) is not None
        }

        proveedores_doc_en_auth = {
            doc_proveedor.get(((c.tipo_documento or "").strip(), (c.numero_documento or "").strip()))
            for c in (auth.contratistas or [])
            if ((c.tipo_documento or "").strip(), (c.numero_documento or "").strip()) in doc_proveedor
            and doc_proveedor.get(((c.tipo_documento or "").strip(), (c.numero_documento or "").strip())) is not None
        }
        proveedores_en_auth.update(proveedores_doc_en_auth)

        if len(proveedores_en_auth) == 1:
            auth.proveedor_id = next(iter(proveedores_en_auth))
