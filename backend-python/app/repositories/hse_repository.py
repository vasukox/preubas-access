...
from sqlalchemy import select, update, func, and_, exists, or_
...
class AutorizacionRepository(BaseRepository[HseAutorizacion]):
    ...
    async def listar_por_sede(
        ...
        tiene_contratista_activo = exists(
            select(HseContratista.id).where(
                HseContratista.autorizacion_id == HseAutorizacion.id,
                HseContratista.deleted_at == None,  # noqa
            )
        )
        query = (
            ...
            .where(
                ...
                HseAutorizacion.deleted_at == None,  # noqa
                or_(
                    HseAutorizacion.estado.in_(("BORRADOR", "PENDIENTE_AUTOGESTION")),
                    tiene_contratista_activo,
                ),
            )
        )
        if estado:
...
class ContratistaRepository(BaseRepository[HseContratista]):
    ...
    async def contar_activos_por_autorizacion(self, autorizacion_id: int) -> int:
        result = await self._db.execute(
            select(func.count(HseContratista.id)).where(
...
class AccesoRepository(BaseRepository[HseAcceso]):
    ...
    async def get_personas_dentro(self, sede_id: int) -> list[HseAcceso]:
        ...
        result = await self._db.execute(
            ...
            .join(
                HseContratista,
                HseContratista.id == HseAcceso.contratista_id,
            )
            .join(
                ultimo_acceso_subq,
                and_(
...
class CumplimientoRepository(BaseRepository[HseCumplimiento]):
    ...
    async def get_ultimo_por_contratista(
        self,
        contratista_id: int,
    ) -> HseCumplimiento | None:
        """Retorna el último cumplimiento (cualquier estado) del contratista."""
        result = await self._db.execute(
            select(HseCumplimiento)
            .options(selectinload(HseCumplimiento.items))
            .where(
