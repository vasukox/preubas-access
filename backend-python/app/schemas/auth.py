...
def _check_email(value: object) -> str:
    """Valida el formato del correo sin hacer consultas DNS.

    email-validator 2.x + dnspython activa verificación DNS por defecto,
    lo que rechaza correos válidos como andres.diaz@gmail.com si el DNS
    tarda o no responde. check_deliverability=False desactiva esa consulta.
    """
    if not isinstance(value, str):
        raise ValueError("El email debe ser una cadena de texto.")
    try:
        info = _validate_email(value.strip(), check_deliverability=False)
        return info.normalized
    except EmailNotValidError as exc:
        raise ValueError(f"Correo inválido: {exc}") from exc


# Tipo reutilizable: valida formato RFC sin DNS
EmailValido = Annotated[str, BeforeValidator(_check_email)]
