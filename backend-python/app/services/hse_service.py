...
class HseService:
    ...
    async def eliminar_contratista(
        ...
        restantes = await self._contratista_repo.contar_activos_por_autorizacion(autorizacion_id)
        if restantes == 0:
            autorizacion = await self._autorizacion_repo.get_by_id(autorizacion_id)
            if autorizacion:
                await self._autorizacion_repo.delete(autorizacion)
                await self._db.commit()
                return
    ...
    async def get_personas_dentro(self, sede_id: int) -> list[dict]:
        ...
        for acceso in accesos:
            ...
            if not contratista or contratista.deleted_at is not None:
                # Evitar romper todo el endpoint por registros históricos huérfanos.
                continue
            autorizacion = await self._autorizacion_repo.get_by_id(
                contratista.autorizacion_id
            )
            if not autorizacion or autorizacion.deleted_at is not None:
                continue
    ...
    async def iniciar_cumplimiento(
        ...
        # Si ya existe un cumplimiento cerrado del mismo día para el contratista,
        # se reutiliza para evitar duplicados al reverificar.
        ultimo = await self._cumplimiento_repo.get_ultimo_por_contratista(data.contratista_id)
        ...
        if (
            ultimo
            and ultimo.estado in ("COMPLETADO", "INCUMPLIMIENTO")
            and ultimo.fecha_inicio is not None
            and ultimo.fecha_inicio.date() == ahora.date()
        ):
            ultimo.sede_id = data.sede_id
            ultimo.encargado_id = usuario_id
            ultimo.estado = "EN_PROGRESO"
            ultimo.observacion_general = None
            ultimo.fecha_inicio = ahora
            ultimo.fecha_cierre = None
            ultimo.firma_digital = None
            await self._cumplimiento_repo.update(ultimo)

            await self._db.execute(
                delete(HseCumplimientoItem).where(HseCumplimientoItem.cumplimiento_id == ultimo.id)
            )

            for i, pregunta in enumerate(preguntas, 1):
                item = HseCumplimientoItem(
                    cumplimiento_id = ultimo.id,
                    pregunta        = pregunta,
                    aplica          = True,
                    cumple          = None,
                    orden           = i,
            ...
            await self._db.commit()
            return await self._cumplimiento_repo.get_with_items(ultimo.id)
