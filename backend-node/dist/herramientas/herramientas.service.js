...
let HerramientasService = class HerramientasService {
    ...
    usuarioPermisoRepo;
    usuarioRolRepo;
    constructor(rolRepo, usuarioRepo, auditLogRepo, perfilRepo, usuarioPermisoRepo, usuarioRolRepo) {
    ...
    async crearUsuario(dto, currentUserId, currentUserName) {
        ...
        if (existe) {
            throw new common_1.ConflictException({ code: 'EMAIL_DUPLICADO', message: `Ya existe un usuario con el email ${dto.email}` });
        }
        ...
        if (esVigilante && !dto.sedeAsignadaId) {
            throw new common_1.BadRequestException({ code: 'SEDE_REQUERIDA', message: 'Los vigilantes deben tener una sede asignada obligatoriamente.' });
        }
    ...
    async asignarRol(id, rolNombre, currentUserId, currentUserName) {
        const usuario = await this.usuarioRepo.findOne({ where: { id }, relations: ['roles', 'roles.rol'] });
        if (!usuario) {
            throw new common_1.NotFoundException({ code: 'USUARIO_NO_ENCONTRADO', message: 'El usuario no existe' });
        }
        ...
        if ((rolNombre === rol_enum_1.RolNombre.VIGILANTE_HSE || rolNombre === rol_enum_1.RolNombre.VIGILANTE_PARKING) && !usuario.sedeAsignadaId) {
            throw new common_1.BadRequestException({ code: 'SEDE_REQUERIDA', message: 'Los vigilantes deben tener una sede asignada obligatoriamente.' });
        }
        return this.usuarioRepo.manager.transaction(async (manager) => {
